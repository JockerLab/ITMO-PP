...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             */
            var acquired = 0
            var ids = listOf(fromIndex, toIndex)
            var fromId = 0
            var toId = 1
            if (fromIndex > toIndex) {
                toId = fromId.also {fromId = toId}
                ids = ids.sorted()
            }
            val accs = ArrayList<AcquiredAccount>()
            while (acquired < 2) {
                val account = acquire(ids[acquired], this) ?: break
                accs.add(account)
                acquired++
            }
            if (acquired == 2) {
                when {
                    amount > accs[fromId].amount -> errorMessage = "Underflow"
                    accs[toId].amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        accs[fromId].newAmount = accs[fromId].amount - amount
                        accs[toId].newAmount = accs[toId].amount + amount
                    }
                }
                completed = true
            }
